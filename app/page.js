...
export default function Home() {
  ...
  // Si se definieran componentes aquí (ej: const SectionPrice = () => ...)
  // React los trataría como nuevos tipos en cada render → remonta → pierde foco
  const handleChange = (field, val) => {
    ...
    if (field === 'showTagline')  setShowTagline(val);
  };
  ...
  if (isMobile) {
    return (
      <div style={{ height:'100dvh', display:'flex', flexDirection:'column', background:'#1C1C1E', overflow:'hidden' }}>
        ...
        {/* Canvas — 50dvh */}
        <div style={{ height:'50dvh', minHeight:220, flexShrink:0, background:'radial-gradient(ellipse at center,#252528 0%,#1C1C1E 70%)', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden', position:'relative' }}>
          ...
          <Suspense fallback={canvasFallback}>
            <CanvasPreview {...canvasProps} isMobile={true} />
          </Suspense>
        </div>
        ...
        {/* Section content — JSX directo, sin wrappers intermedios */}
              ...
              <div className="flex gap-2">
                {LOGO_POSITIONS.map(p => (
                  <button key={p.id} onClick={() => setLogoPosition(p.id)}
                    ...
                    style={{ background:logoPosition===p.id?'rgba(196,30,42,0.15)':'rgba(255,255,255,0.04)', border:`1.5px solid ${logoPosition===p.id?'#C41E2A':'#3A3A3E'}` }}>
          ...
          {mobileTab === 'price' &&
            <section style={panelStyle}>
              <PriceEditor
                ...
                oldPrice={oldPrice} currency={currency}
                badgeText={badgeText} tagline={tagline} showTagline={showTagline}
  ...
  return (
    <main style={{ display:'grid', gridTemplateColumns:'375px 1fr', gridTemplateRows:'auto 1fr', height:'100vh', overflow:'hidden', background:'#1C1C1E' }}>
      ...
      {/* Left panel — JSX directo sin wrappers intermedios */}
      ...
      {/* Canvas */}
      <section style={{ overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center', background:'radial-gradient(ellipse at center,#252528 0%,#1C1C1E 70%)', position:'relative' }}>
        <div style={{ position:'absolute', inset:0, backgroundImage:'linear-gradient(rgba(255,255,255,0.02) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,0.02) 1px,transparent 1px)', backgroundSize:'40px 40px', pointerEvents:'none' }} />
        <Suspense fallback={canvasFallback}>
          <CanvasPreview {...canvasProps} isMobile={false} />
        </Suspense>
      </section>
    </main>
  );
